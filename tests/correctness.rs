...
fn test_interface_checking() {
    use sqrt_space_sim::blocking::{BlockSummary, MovementLog, InterfaceChecker};
...
fn test_boundary_reconstruction() {
    ...
    assert_eq!(config2_boundary.state(), exit_state, 
               "Block 2 should start from block 1's exit state");
    assert_eq!(config2_boundary.head_positions(), exit_heads,
    ...
    assert_eq!(exit_state, summary2.entry_state(),
               "Block 1 exit state should match block 2 entry state");
    assert_eq!(exit_heads, summary2.entry_heads(),
    ...
    assert!(InterfaceChecker::check(&summary1, &summary2).unwrap(),
