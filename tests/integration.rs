...
fn test_end_to_end_simulation() {
    ...
    assert!(result.satisfies_bound(&config), "Space bound should be satisfied");
...
fn test_large_computation() {
    ...
    let result = simulator.run(&input).expect("Large simulation should succeed");
...
fn test_multiple_blocks() {
    ...
    let result = simulator.run(&input).expect("Multi-block simulation should succeed");
    ...
    assert!(config.num_blocks > 1, "Should have multiple blocks for t={}", time_bound);
}
...
fn test_large_multi_block_simulation() {
    ...
    let mut simulator = Simulator::new(machine, config.clone());
    let result = simulator.run(&input).expect("Large multi-block simulation should succeed");
