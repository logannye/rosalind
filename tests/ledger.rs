...
fn test_ledger_all_merges_complete() {
    ...
    assert!(
        !ledger.all_merges_complete(),
        "No merges should be complete initially"
    );
    ...
    assert!(
        stats.2 >= 3,
        "Should have at least 3 merges complete (got {})",
        stats.2
    );
}
...
fn test_ledger_is_merge_ready() {
    ...
    // Initially not ready
    assert!(
        !ledger.is_merge_ready(root),
        "Merge should not be ready initially"
    );
    ...
    assert!(
        !ledger.is_merge_ready(root),
        "Merge should not be ready with only left"
    );
    ...
    assert!(
        ledger.is_merge_ready(root),
        "Merge should be ready with both complete"
    );
}
