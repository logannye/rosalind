...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, &time_bound) in time_bounds.iter().enumerate() {
        ...
        if time_bound >= 1_000_000 {
            println!(
                "Running test for t={} (this may take a while)...",
                time_bound
            );
        }
        ...
        // Create metrics
        let space_profile = result.space_profile.clone();
        let mut metrics = TestMetrics::new(time_bound, &config, result.space_used, elapsed);
        ...
        if let Some(profile) = space_profile {
            println!(
                "    components: leaf={} stack={} ledger={}",
                format_number(profile.leaf_buffer_max),
                format_number(profile.stack_depth_max),
                format_number(profile.ledger_size)
            );
        }
    }
    ...
    let all_within_bound = all_metrics.iter().all(|m| m.space_used <= m.sqrt_t_bound);
    ...
    let scaling_ratios: Vec<f64> = all_metrics.iter().filter_map(|m| m.scaling_ratio).collect();

    if !scaling_ratios.is_empty() {
        let avg_scaling = scaling_ratios.iter().sum::<f64>() / scaling_ratios.len() as f64;
        println!(
            "✓ Space scaling: {:.2}x average (expected: ~2.00x for 4x time increase)",
            avg_scaling
        );
    ...
    if let Some(last) = all_metrics.last() {
        ...
        println!(
            "✓ Efficiency: {:.2}% space savings vs naive O(t) at t={}",
            efficiency_pct, last.time_bound
        );
        println!(
            "✓ Largest test: t={} completed in {:.2}s",
    ...
    if csv_output {
        ...
        for m in &all_metrics {
            ...
            println!(
                ...
                m.space_used,
                m.sqrt_t_bound,
                m.space_efficiency_ratio,
                m.execution_time_secs,
                scaling
            );
        }
    }
