...
pub use transition::{Transition, Move};
...
impl TuringMachine {
    ...
    pub fn step(&self, config: &mut Configuration) -> Result<(), SimulationError> {
        ...
        let transition = self.transition(config.state(), &symbols)
            .ok_or_else(|| SimulationError::InvalidMachine(
                format!("No transition for state {} and symbols {:?}", config.state(), symbols)
            ))?;
...
impl TuringMachineBuilder {
    ...
    pub fn add_transition(
        ...
        self.transitions.push((from_state, read_symbols, transition));
        self
    }
    ...
    pub fn build(self) -> Result<TuringMachine, SimulationError> {
        ...
        let alphabet = self.alphabet.ok_or_else(|| {
            SimulationError::InvalidMachine("Alphabet not set".to_string())
        })?;
        
        let initial_state = self.initial_state.unwrap_or(0);
        let accept_state = self.accept_state.ok_or_else(|| {
            SimulationError::InvalidMachine("Accept state not set".to_string())
        })?;
        let reject_state = self.reject_state.ok_or_else(|| {
            SimulationError::InvalidMachine("Reject state not set".to_string())
        })?;
        ...
        for (from_state, read_symbols, transition) in self.transitions {
            ...
            if read_symbols.len() != num_tapes + 1 {
                return Err(SimulationError::InvalidMachine(
                    format!("Transition read symbols length {} doesn't match num_tapes+1 {}", 
                            read_symbols.len(), num_tapes + 1)
                ));
            }
            ...
            if transition.writes.len() != num_tapes {
                return Err(SimulationError::InvalidMachine(
                    format!("Transition write symbols length {} doesn't match num_tapes {}", 
                            transition.writes.len(), num_tapes)
                ));
            }
            ...
            if transition.moves.len() != num_tapes + 1 {
                return Err(SimulationError::InvalidMachine(
                    format!("Transition moves length {} doesn't match num_tapes+1 {}", 
                            transition.moves.len(), num_tapes + 1)
                ));
            }
